...
impl AshInstance {
    pub(crate) fn new_device(self: &Arc<Self>) -> Result<Arc<AshDevice>> {
        let (physical_device, indices, device_extensions) = pick_physical_device(self, None)?;
        AshDevice::new(self, physical_device, indices, device_extensions)
    }

    pub(crate) fn new_device_for_surface(
        self: &Arc<Self>,
        surface: &Surface,
    ) -> Result<Arc<AshDevice>> {
        let (physical_device, indices, device_extensions) =
            pick_physical_device(self, Some(surface))?;
        AshDevice::new(self, physical_device, indices, device_extensions)
    }
}
...
impl AshDevice {
    fn new(
        ...
        let (device_raw, queues) = create_logical_device(
            instance,
            physical_device,
            indices,
...
fn pick_physical_device(
    ...
    for (i, &physical_device) in physical_devices.iter().enumerate() {
        log_device_infos(instance, physical_device, i);

        if let Some((indices, extensions)) =
            check_physical_device_suitable(instance, physical_device, for_surface)
        {
            if result.is_none() {
...
fn check_physical_device_suitable(
    ...
    let indices = QueueFamilyIndices::from_physical_device(instance, physical_device, for_surface)?;
    let extensions = get_device_extensions(instance, physical_device).ok()?;

    if let Some(surface) = for_surface {
        if !extensions.contains(&khr::Swapchain::name())
            || surface.query_swapchain_support(physical_device).is_none()
        {
            return None;
        }
    }
...
fn create_logical_device<'a>(
    ...
    _create_logical_device(instance, physical_device, indices, &extensions_ptr)
}
    ...
    fn from_physical_device(
        ...
        for (i, queue_family) in queue_families.iter().enumerate() {
            ...
            if indices.present.is_none()
                ...
                && for_surface
                    .unwrap()
                    .get_physical_device_surface_support(physical_device, i)
            {
                indices.present = Some(i);
            }
