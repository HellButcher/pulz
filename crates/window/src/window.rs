...
use ecs::{module::Module, prelude::Schedule, resource::Resources};
...
impl WindowDescriptor {
    ...
    pub fn new() -> Self {
        Self {
            position: Point2::new(i32::MIN, i32::MIN),
            size: Size2::ZERO,
            scale_factor: 1.0,
...
impl Module for WindowDescriptor {
    type Output = WindowId;

    fn install(self, resources: &mut Resources, _schedule: &mut Schedule) -> WindowId {
        let windows_id = resources.init::<Windows>();
        resources
            .borrow_res_mut_id(windows_id)
            .unwrap()
            .create(self)
    }
}
...
pub type RawWindowHandles = WindowsMirror<Weak<dyn HasRawWindowHandle>>;
