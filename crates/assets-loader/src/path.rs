...
impl<T: ?Sized + AsRef<str>> From<&T> for AssetPathBuf {
    #[inline]
    fn from(s: &T) -> AssetPathBuf {
        AssetPathBuf::from(s.as_ref().to_string())
    }
}

impl From<String> for AssetPathBuf {
    #[inline]
    fn from(s: String) -> AssetPathBuf {
        AssetPathBuf(s)
    }
}
impl From<AssetPathBuf> for String {
    #[inline]
    fn from(path_buf: AssetPathBuf) -> String {
        path_buf.0
    }
}
...
impl AssetPath {
    ...
    pub fn is_absolute(&self) -> bool {
        self.0.starts_with(AssetPath::is_seperator)
    }
    ...
    pub fn is_directory(&self) -> bool {
        ...
        path.ends_with(AssetPath::is_seperator)
    }
    ...
    fn split_parent_fragment(&self) -> (Option<&str>, &str, Option<&str>) {
        ...
        if let Some(pos) = path
            .trim_end_matches(AssetPath::is_seperator)
            .rfind(AssetPath::is_seperator)
        {
    ...
    pub fn parent(&self) -> Option<&AssetPath> {
        ...
        parent.map(AssetPath::new)
    }

    pub fn file_name(&self) -> Option<&str> {
        let (_parent, file, _frag) = self.split_parent_fragment();
        let file = file.trim_end_matches(AssetPath::is_seperator);
        if file.is_empty() {
            None
        } else {
            Some(file)
    ...
    fn split_parent_file_extension_fragment(
        ...
        if file.ends_with(AssetPath::is_seperator) {
            let filename = file.trim_end_matches(AssetPath::is_seperator);
    ...
    pub fn join<P: AsRef<AssetPath>>(&self, path: P) -> AssetPathBuf {
        self._join(path.as_ref())
    }

    fn _join(&self, path: &AssetPath) -> AssetPathBuf {
...
impl AsRef<AssetPath> for AssetPath {
    #[inline]
