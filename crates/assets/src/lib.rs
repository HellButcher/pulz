...
use schedule::system::IntoSystem;
use slotmap::{KeyData, SlotMap};
use std::hash::{Hash, Hasher};
use std::{fmt::Debug, marker::PhantomData};

use schedule::event::{EventWriter, Events};
use schedule::resource::{ResMut, Resources};
...
unsafe impl<T> slotmap::Key for Handle<T> {
    #[inline]
...
impl<T> Assets<T> {
        ...
        T: Send + Sync + 'static,
    {
        if res.try_init::<Self>().is_ok() {
            Events::<AssetEvent<T>>::install_into(res, schedule);
            schedule.add_system(Self::update_system.after(AssetSystemLabel::UpdateAssets));
        }
    }
}

#[derive(Copy, Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum AssetSystemLabel {
    LoadAssets,
    UpdateAssets,
}
